...
class SimCTGPretraining(nn.Module):
    ...
    def compute_mle_loss(self, input_ids, labels):
        ...
        return mle_loss
    """
    # 기존 comput cl loss 함수 -> 수정 
    def compute_contrastive_loss(self, score_matrix, margin): # 수정필요 함수 
        '''
    ...
    def compute_contrastive_loss(self, cosine_scores_plus, cosine_scores_minus, margin): # 수정필요 함수 
        '''
           margin: predefined margin to push similarity score away
           score_matrix: bsz x seqlen x seqlen; cosine similarity matrix
           input_ids: bsz x seqlen
        '''
        bsz, seqlen, _ = cosine_scores_plus.size()
        #gold_score = torch.diagonal(score_matrix, offset=0, dim1=1, dim2=2) # bsz x seqlen
        #gold_score = torch.unsqueeze(gold_score, -1)
        #assert gold_score.size() == torch.Size([bsz, seqlen, 1])

        #### anchor와 anchor plus 를 가깝게 하고싶으면 이것. 
        #difference_matrix = cosine_scores_plus - cosine_scores_minus

        #### anchor와 anchor minus 를 가깝게 하고싶으면 이것. 
        difference_matrix = cosine_scores_minus- cosine_scores_plus
    ...
    def forward(self, input_ids, labels, margin):
      ...
      '''
    def forward(self, input_ids, input_ids_plus, input_ids_minus, labels, margin): # input_ids_plus, input_ids_minus 추가 
        bsz, seqlen = input_ids.size()
        outputs = self.model(input_ids=input_ids, output_hidden_states=True)
        # plus hidden
        ...
        last_hidden_states = outputs.hidden_states[-1]
        # 추가
        last_hidden_states_plus = outputs_plus.hidden_states[-1]
        last_hidden_states_minus = outputs_minus.hidden_states[-1]
        assert last_hidden_states.size() == torch.Size([bsz, seqlen, self.embed_dim])
        mle_loss = train_fct(logits.view(-1, self.vocab_size), labels.view(-1))
        # 추가? mle loss 는 어떻게 해야하지 -> 그대로 outpus 와 label 로 가자. 

        # comput cl loss (anchor)
        norm_rep = last_hidden_states / last_hidden_states.norm(dim=2, keepdim=True)
        # (anchor+) 
        ...
        cosine_scores_plus = torch.matmul(norm_rep, norm_rep_plus.transpose(1, 2))
        assert cosine_scores.size() == torch.Size([bsz, seqlen, seqlen])

        # anchor 와 anchor+ cosine 유사도 행렬 
        cosine_scores_minus = torch.matmul(norm_rep, norm_rep_minus.transpose(1, 2))
        assert cosine_scores.size() == torch.Size([bsz, seqlen, seqlen])

        if self.use_cl_loss:
            cl_loss = self.compute_contrastive_loss(cosine_scores_plus, cosine_scores_minus, margin)
        else:
            cl_loss = torch.Tensor([0.])
            if input_ids.is_cuda:
                cl_loss = cl_loss.cuda(input_ids.get_device())
