...
if __name__ == '__main__':
    ...
    while effective_batch_acm < total_steps:
        ...
        for batch_data in data:
            input_ids, input_ids_plus, input_ids_minus = batch_data  # ???? 
            ...
            if cuda_available:
                ...
                input_ids_plus = input_ids_plus.cuda(device)
                input_ids_minus = input_ids_minus.cuda(device)
                labels = input_ids.cuda(device) #???
            #mle_loss, cl_loss = model(train_batch_inpnuts, train_batch_labels, args.margin)
            mle_loss, cl_loss = model(input_ids, input_ids_plus, input_ids_minus, labels, args.margin)
            if use_cl_loss:
    ...
    while effective_batch_acm < total_steps:
        # for train_batch_inputs, train_batch_labels in data:
        for batch_data in data:
            input_ids, input_ids_plus, input_ids_minus, labels = batch_data  # Unpack the batch data
            if effective_batch_acm >= total_steps:
                break  # stop training
            ...
            if cuda_available:
                ...
                input_ids_plus = input_ids_plus.cuda(device)
                input_ids_minus = input_ids_minus.cuda(device)
                labels = labels.cuda(device)  # Use the labels from the batch_data
